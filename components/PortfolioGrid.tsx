...
export const PortfolioGrid = (props:PortfolioGridProps) => {
  ...
  return (
    <Flex flexDirection={{base:'column', md:reverse?'row-reverse': 'row'}} mb={20} position='relative' width={{base:'100%', md:'80%'}}>
        ...
        {showIndex && showIndex === cur && <Flex position='absolute' flexDir='column' bg={{base:secondaryColor, md:'none'}} fontSize={{base:12, md:16}} p={{base:2, md:5}} top={{base:4, md:10}} width={{base:'95%', md:'600px'}} _hover={{diaplay:'flex'}} left={{base:2, md:left}} display={{md:'flex'}} flexGrow={1} alignItems={reverse?'flex-start':'flex-end'}>
